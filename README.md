...
排程設定為每 10 分鐘檢查一次 `@oorainielove520oo` 的公開 Threads 貼文，依情報種類分流到 Discord：
...
之後排程會在每小時的第 7、17、27、37、47、57 分嘗試自動檢查；程式更新合併到 `main` 後也會立即檢查一次。GitHub Actions 的排程可能因平台壅塞延遲，因此程式會保留時間水位並在下一次成功執行時補抓期間的新貼文。
