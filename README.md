...
排程設定為每 1 小時檢查一次 `@oorainielove520oo` 的公開 Threads 貼文，依情報種類分流到 Discord：
...
- `🎟️ 新抽獎獎池`：辨識新獎池、抽獎獎池、藍鑽池與風物獎池爆料，整理相關段落並發到更新資料頻道
...
- 第一次啟動只發送最新一篇符合的貼文
- 中斷後依時間順序補送遺漏的新貼文
- 新增分類後會檢查最近 24 小時貼文，只補送尚未發布的分類；已發布的橡木螢石等內容不會重複
- 同一篇貼文可同時拆成多種情報並分流到不同頻道，例如橡木螢石送採集頻道、食譜送更新資料、兌換碼送兌換碼頻道；以「貼文 ID＋情報分類」各自防止重複
- 只擷取貼文的大圖，排除頭像與介面圖示
...
之後排程會在每小時的第 17 分嘗試自動檢查；程式更新合併到 `main` 後也會立即檢查一次。GitHub Actions 的排程可能因平台壅塞延遲，因此程式會保留時間水位並在下一次成功執行時補抓期間的新貼文。
